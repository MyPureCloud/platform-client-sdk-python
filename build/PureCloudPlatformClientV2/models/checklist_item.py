# coding: utf-8

"""
Copyright 2016 SmartBear Software

    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at

        http://www.apache.org/licenses/LICENSE-2.0

    Unless required by applicable law or agreed to in writing, software
    distributed under the License is distributed on an "AS IS" BASIS,
    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
    See the License for the specific language governing permissions and
    limitations under the License.

    Ref: https://github.com/swagger-api/swagger-codegen
"""

from datetime import datetime
from datetime import date
from pprint import pformat
import re
import json

from ..utils import sanitize_for_serialization

# type hinting support
from typing import TYPE_CHECKING
from typing import List
from typing import Dict


class ChecklistItem(object):
    """
    NOTE: This class is auto generated by the swagger code generator program.
    Do not edit the class manually.
    """
    def __init__(self) -> None:
        """
        ChecklistItem - a model defined in Swagger

        :param dict swaggerTypes: The key is attribute name
                                  and the value is attribute type.
        :param dict attributeMap: The key is attribute name
                                  and the value is json key in definition.
        """
        self.swagger_types = {
            'id': 'str',
            'name': 'str',
            'description': 'str',
            'automated_check_enabled': 'bool',
            'important': 'bool',
            'state_from_model': 'str',
            'state_from_agent': 'str',
            'date_last_modified_by_model': 'datetime',
            'date_last_modified_by_agent': 'datetime',
            'last_modified_in_acw': 'bool',
            'self_uri': 'str'
        }

        self.attribute_map = {
            'id': 'id',
            'name': 'name',
            'description': 'description',
            'automated_check_enabled': 'automatedCheckEnabled',
            'important': 'important',
            'state_from_model': 'stateFromModel',
            'state_from_agent': 'stateFromAgent',
            'date_last_modified_by_model': 'dateLastModifiedByModel',
            'date_last_modified_by_agent': 'dateLastModifiedByAgent',
            'last_modified_in_acw': 'lastModifiedInAcw',
            'self_uri': 'selfUri'
        }

        self._id = None
        self._name = None
        self._description = None
        self._automated_check_enabled = None
        self._important = None
        self._state_from_model = None
        self._state_from_agent = None
        self._date_last_modified_by_model = None
        self._date_last_modified_by_agent = None
        self._last_modified_in_acw = None
        self._self_uri = None

    @property
    def id(self) -> str:
        """
        Gets the id of this ChecklistItem.
        ID of the checklist item.

        :return: The id of this ChecklistItem.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id: str) -> None:
        """
        Sets the id of this ChecklistItem.
        ID of the checklist item.

        :param id: The id of this ChecklistItem.
        :type: str
        """
        

        self._id = id

    @property
    def name(self) -> str:
        """
        Gets the name of this ChecklistItem.
        Name of the checklist item.

        :return: The name of this ChecklistItem.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name: str) -> None:
        """
        Sets the name of this ChecklistItem.
        Name of the checklist item.

        :param name: The name of this ChecklistItem.
        :type: str
        """
        

        self._name = name

    @property
    def description(self) -> str:
        """
        Gets the description of this ChecklistItem.
        Description of the checklist item.

        :return: The description of this ChecklistItem.
        :rtype: str
        """
        return self._description

    @description.setter
    def description(self, description: str) -> None:
        """
        Sets the description of this ChecklistItem.
        Description of the checklist item.

        :param description: The description of this ChecklistItem.
        :type: str
        """
        

        self._description = description

    @property
    def automated_check_enabled(self) -> bool:
        """
        Gets the automated_check_enabled of this ChecklistItem.
        Flag to indicate whether automated check is enabled for this checklist item.

        :return: The automated_check_enabled of this ChecklistItem.
        :rtype: bool
        """
        return self._automated_check_enabled

    @automated_check_enabled.setter
    def automated_check_enabled(self, automated_check_enabled: bool) -> None:
        """
        Sets the automated_check_enabled of this ChecklistItem.
        Flag to indicate whether automated check is enabled for this checklist item.

        :param automated_check_enabled: The automated_check_enabled of this ChecklistItem.
        :type: bool
        """
        

        self._automated_check_enabled = automated_check_enabled

    @property
    def important(self) -> bool:
        """
        Gets the important of this ChecklistItem.
        Flag to indicate whether this checklist item is marked as important.

        :return: The important of this ChecklistItem.
        :rtype: bool
        """
        return self._important

    @important.setter
    def important(self, important: bool) -> None:
        """
        Sets the important of this ChecklistItem.
        Flag to indicate whether this checklist item is marked as important.

        :param important: The important of this ChecklistItem.
        :type: bool
        """
        

        self._important = important

    @property
    def state_from_model(self) -> str:
        """
        Gets the state_from_model of this ChecklistItem.
        Checklist state as evaluated by the model.

        :return: The state_from_model of this ChecklistItem.
        :rtype: str
        """
        return self._state_from_model

    @state_from_model.setter
    def state_from_model(self, state_from_model: str) -> None:
        """
        Sets the state_from_model of this ChecklistItem.
        Checklist state as evaluated by the model.

        :param state_from_model: The state_from_model of this ChecklistItem.
        :type: str
        """
        if isinstance(state_from_model, int):
            state_from_model = str(state_from_model)
        allowed_values = ["Ticked", "Unticked"]
        if state_from_model.lower() not in map(str.lower, allowed_values):
            # print("Invalid value for state_from_model -> " + state_from_model)
            self._state_from_model = "outdated_sdk_version"
        else:
            self._state_from_model = state_from_model

    @property
    def state_from_agent(self) -> str:
        """
        Gets the state_from_agent of this ChecklistItem.
        Checklist state as evaluated by the agent.

        :return: The state_from_agent of this ChecklistItem.
        :rtype: str
        """
        return self._state_from_agent

    @state_from_agent.setter
    def state_from_agent(self, state_from_agent: str) -> None:
        """
        Sets the state_from_agent of this ChecklistItem.
        Checklist state as evaluated by the agent.

        :param state_from_agent: The state_from_agent of this ChecklistItem.
        :type: str
        """
        if isinstance(state_from_agent, int):
            state_from_agent = str(state_from_agent)
        allowed_values = ["Ticked", "Unticked"]
        if state_from_agent.lower() not in map(str.lower, allowed_values):
            # print("Invalid value for state_from_agent -> " + state_from_agent)
            self._state_from_agent = "outdated_sdk_version"
        else:
            self._state_from_agent = state_from_agent

    @property
    def date_last_modified_by_model(self) -> datetime:
        """
        Gets the date_last_modified_by_model of this ChecklistItem.
        Date when the checklist item was last modified by the model. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :return: The date_last_modified_by_model of this ChecklistItem.
        :rtype: datetime
        """
        return self._date_last_modified_by_model

    @date_last_modified_by_model.setter
    def date_last_modified_by_model(self, date_last_modified_by_model: datetime) -> None:
        """
        Sets the date_last_modified_by_model of this ChecklistItem.
        Date when the checklist item was last modified by the model. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :param date_last_modified_by_model: The date_last_modified_by_model of this ChecklistItem.
        :type: datetime
        """
        

        self._date_last_modified_by_model = date_last_modified_by_model

    @property
    def date_last_modified_by_agent(self) -> datetime:
        """
        Gets the date_last_modified_by_agent of this ChecklistItem.
        Date when the checklist item was last modified by the agent. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :return: The date_last_modified_by_agent of this ChecklistItem.
        :rtype: datetime
        """
        return self._date_last_modified_by_agent

    @date_last_modified_by_agent.setter
    def date_last_modified_by_agent(self, date_last_modified_by_agent: datetime) -> None:
        """
        Sets the date_last_modified_by_agent of this ChecklistItem.
        Date when the checklist item was last modified by the agent. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :param date_last_modified_by_agent: The date_last_modified_by_agent of this ChecklistItem.
        :type: datetime
        """
        

        self._date_last_modified_by_agent = date_last_modified_by_agent

    @property
    def last_modified_in_acw(self) -> bool:
        """
        Gets the last_modified_in_acw of this ChecklistItem.
        Flag to indicate whether this checklist item was modified in ACW.

        :return: The last_modified_in_acw of this ChecklistItem.
        :rtype: bool
        """
        return self._last_modified_in_acw

    @last_modified_in_acw.setter
    def last_modified_in_acw(self, last_modified_in_acw: bool) -> None:
        """
        Sets the last_modified_in_acw of this ChecklistItem.
        Flag to indicate whether this checklist item was modified in ACW.

        :param last_modified_in_acw: The last_modified_in_acw of this ChecklistItem.
        :type: bool
        """
        

        self._last_modified_in_acw = last_modified_in_acw

    @property
    def self_uri(self) -> str:
        """
        Gets the self_uri of this ChecklistItem.
        The URI for this object

        :return: The self_uri of this ChecklistItem.
        :rtype: str
        """
        return self._self_uri

    @self_uri.setter
    def self_uri(self, self_uri: str) -> None:
        """
        Sets the self_uri of this ChecklistItem.
        The URI for this object

        :param self_uri: The self_uri of this ChecklistItem.
        :type: str
        """
        

        self._self_uri = self_uri

    def to_dict(self):
        """
        Returns the model properties as a dict
        """
        result = {}

        for attr, _ in self.swagger_types.items():
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value

        return result

    def to_json(self):
        """
        Returns the model as raw JSON
        """
        return json.dumps(sanitize_for_serialization(self.to_dict()))

    def to_str(self):
        """
        Returns the string representation of the model
        """
        return pformat(self.to_dict())

    def __repr__(self):
        """
        For `print` and `pprint`
        """
        return self.to_str()

    def __eq__(self, other):
        """
        Returns true if both objects are equal
        """
        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """
        Returns true if both objects are not equal
        """
        return not self == other

