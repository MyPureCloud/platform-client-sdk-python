# coding: utf-8

"""
Copyright 2016 SmartBear Software

    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at

        http://www.apache.org/licenses/LICENSE-2.0

    Unless required by applicable law or agreed to in writing, software
    distributed under the License is distributed on an "AS IS" BASIS,
    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
    See the License for the specific language governing permissions and
    limitations under the License.

    Ref: https://github.com/swagger-api/swagger-codegen
"""

from datetime import datetime
from datetime import date
from pprint import pformat
from six import iteritems
import re
import json

from ..utils import sanitize_for_serialization

# type hinting support
from typing import TYPE_CHECKING
from typing import List
from typing import Dict


class UserState(object):
    """
    NOTE: This class is auto generated by the swagger code generator program.
    Do not edit the class manually.
    """
    def __init__(self) -> None:
        """
        UserState - a model defined in Swagger

        :param dict swaggerTypes: The key is attribute name
                                  and the value is attribute type.
        :param dict attributeMap: The key is attribute name
                                  and the value is json key in definition.
        """
        self.swagger_types = {
            'state': 'str',
            'version': 'int',
            'state_change_reason': 'str',
            'state_change_date': 'datetime'
        }

        self.attribute_map = {
            'state': 'state',
            'version': 'version',
            'state_change_reason': 'stateChangeReason',
            'state_change_date': 'stateChangeDate'
        }

        self._state = None
        self._version = None
        self._state_change_reason = None
        self._state_change_date = None

    @property
    def state(self) -> str:
        """
        Gets the state of this UserState.
        User's current state.

        :return: The state of this UserState.
        :rtype: str
        """
        return self._state

    @state.setter
    def state(self, state: str) -> None:
        """
        Sets the state of this UserState.
        User's current state.

        :param state: The state of this UserState.
        :type: str
        """
        if isinstance(state, int):
            state = str(state)
        allowed_values = ["active", "inactive", "deleted"]
        if state.lower() not in map(str.lower, allowed_values):
            # print("Invalid value for state -> " + state)
            self._state = "outdated_sdk_version"
        else:
            self._state = state

    @property
    def version(self) -> int:
        """
        Gets the version of this UserState.
        Version of this user.

        :return: The version of this UserState.
        :rtype: int
        """
        return self._version

    @version.setter
    def version(self, version: int) -> None:
        """
        Sets the version of this UserState.
        Version of this user.

        :param version: The version of this UserState.
        :type: int
        """
        

        self._version = version

    @property
    def state_change_reason(self) -> str:
        """
        Gets the state_change_reason of this UserState.
        Reason for a change in the user's state.

        :return: The state_change_reason of this UserState.
        :rtype: str
        """
        return self._state_change_reason

    @state_change_reason.setter
    def state_change_reason(self, state_change_reason: str) -> None:
        """
        Sets the state_change_reason of this UserState.
        Reason for a change in the user's state.

        :param state_change_reason: The state_change_reason of this UserState.
        :type: str
        """
        if isinstance(state_change_reason, int):
            state_change_reason = str(state_change_reason)
        allowed_values = ["Voluntary", "Seasonal", "Leave", "Performance", "Conduct", "Unknown"]
        if state_change_reason.lower() not in map(str.lower, allowed_values):
            # print("Invalid value for state_change_reason -> " + state_change_reason)
            self._state_change_reason = "outdated_sdk_version"
        else:
            self._state_change_reason = state_change_reason

    @property
    def state_change_date(self) -> datetime:
        """
        Gets the state_change_date of this UserState.
        Date that the state was last changed. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :return: The state_change_date of this UserState.
        :rtype: datetime
        """
        return self._state_change_date

    @state_change_date.setter
    def state_change_date(self, state_change_date: datetime) -> None:
        """
        Sets the state_change_date of this UserState.
        Date that the state was last changed. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :param state_change_date: The state_change_date of this UserState.
        :type: datetime
        """
        

        self._state_change_date = state_change_date

    def to_dict(self):
        """
        Returns the model properties as a dict
        """
        result = {}

        for attr, _ in iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value

        return result

    def to_json(self):
        """
        Returns the model as raw JSON
        """
        return json.dumps(sanitize_for_serialization(self.to_dict()))

    def to_str(self):
        """
        Returns the string representation of the model
        """
        return pformat(self.to_dict())

    def __repr__(self):
        """
        For `print` and `pprint`
        """
        return self.to_str()

    def __eq__(self, other):
        """
        Returns true if both objects are equal
        """
        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """
        Returns true if both objects are not equal
        """
        return not self == other

