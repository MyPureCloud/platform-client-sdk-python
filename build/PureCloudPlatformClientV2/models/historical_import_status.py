# coding: utf-8

"""
Copyright 2016 SmartBear Software

    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at

        http://www.apache.org/licenses/LICENSE-2.0

    Unless required by applicable law or agreed to in writing, software
    distributed under the License is distributed on an "AS IS" BASIS,
    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
    See the License for the specific language governing permissions and
    limitations under the License.

    Ref: https://github.com/swagger-api/swagger-codegen
"""

from datetime import datetime
from datetime import date
from pprint import pformat
from six import iteritems
import re
import json

from ..utils import sanitize_for_serialization

# type hinting support
from typing import TYPE_CHECKING
from typing import List
from typing import Dict


class HistoricalImportStatus(object):
    """
    NOTE: This class is auto generated by the swagger code generator program.
    Do not edit the class manually.
    """
    def __init__(self) -> None:
        """
        HistoricalImportStatus - a model defined in Swagger

        :param dict swaggerTypes: The key is attribute name
                                  and the value is attribute type.
        :param dict attributeMap: The key is attribute name
                                  and the value is json key in definition.
        """
        self.swagger_types = {
            'request_id': 'str',
            'date_import_ended': 'datetime',
            'date_import_started': 'datetime',
            'status': 'str',
            'error': 'str',
            'date_created': 'datetime',
            'date_modified': 'datetime',
            'active': 'bool',
            'type': 'str'
        }

        self.attribute_map = {
            'request_id': 'requestId',
            'date_import_ended': 'dateImportEnded',
            'date_import_started': 'dateImportStarted',
            'status': 'status',
            'error': 'error',
            'date_created': 'dateCreated',
            'date_modified': 'dateModified',
            'active': 'active',
            'type': 'type'
        }

        self._request_id = None
        self._date_import_ended = None
        self._date_import_started = None
        self._status = None
        self._error = None
        self._date_created = None
        self._date_modified = None
        self._active = None
        self._type = None

    @property
    def request_id(self) -> str:
        """
        Gets the request_id of this HistoricalImportStatus.
        Request id of the historical import in the organization

        :return: The request_id of this HistoricalImportStatus.
        :rtype: str
        """
        return self._request_id

    @request_id.setter
    def request_id(self, request_id: str) -> None:
        """
        Sets the request_id of this HistoricalImportStatus.
        Request id of the historical import in the organization

        :param request_id: The request_id of this HistoricalImportStatus.
        :type: str
        """
        

        self._request_id = request_id

    @property
    def date_import_ended(self) -> datetime:
        """
        Gets the date_import_ended of this HistoricalImportStatus.
        The last day of the data you are importing. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :return: The date_import_ended of this HistoricalImportStatus.
        :rtype: datetime
        """
        return self._date_import_ended

    @date_import_ended.setter
    def date_import_ended(self, date_import_ended: datetime) -> None:
        """
        Sets the date_import_ended of this HistoricalImportStatus.
        The last day of the data you are importing. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :param date_import_ended: The date_import_ended of this HistoricalImportStatus.
        :type: datetime
        """
        

        self._date_import_ended = date_import_ended

    @property
    def date_import_started(self) -> datetime:
        """
        Gets the date_import_started of this HistoricalImportStatus.
        The first day of the data you are importing. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :return: The date_import_started of this HistoricalImportStatus.
        :rtype: datetime
        """
        return self._date_import_started

    @date_import_started.setter
    def date_import_started(self, date_import_started: datetime) -> None:
        """
        Sets the date_import_started of this HistoricalImportStatus.
        The first day of the data you are importing. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :param date_import_started: The date_import_started of this HistoricalImportStatus.
        :type: datetime
        """
        

        self._date_import_started = date_import_started

    @property
    def status(self) -> str:
        """
        Gets the status of this HistoricalImportStatus.
        Status of the historical import in the organization.

        :return: The status of this HistoricalImportStatus.
        :rtype: str
        """
        return self._status

    @status.setter
    def status(self, status: str) -> None:
        """
        Sets the status of this HistoricalImportStatus.
        Status of the historical import in the organization.

        :param status: The status of this HistoricalImportStatus.
        :type: str
        """
        if isinstance(status, int):
            status = str(status)
        allowed_values = ["Initiated", "InProgress", "Pending", "Success", "Failed", "Cancelled", "Purged", "PurgePending"]
        if status.lower() not in map(str.lower, allowed_values):
            # print("Invalid value for status -> " + status)
            self._status = "outdated_sdk_version"
        else:
            self._status = status

    @property
    def error(self) -> str:
        """
        Gets the error of this HistoricalImportStatus.
        Error occured if the status of the import is failed

        :return: The error of this HistoricalImportStatus.
        :rtype: str
        """
        return self._error

    @error.setter
    def error(self, error: str) -> None:
        """
        Sets the error of this HistoricalImportStatus.
        Error occured if the status of the import is failed

        :param error: The error of this HistoricalImportStatus.
        :type: str
        """
        

        self._error = error

    @property
    def date_created(self) -> datetime:
        """
        Gets the date_created of this HistoricalImportStatus.
        Date in which the historical import is initiated. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :return: The date_created of this HistoricalImportStatus.
        :rtype: datetime
        """
        return self._date_created

    @date_created.setter
    def date_created(self, date_created: datetime) -> None:
        """
        Sets the date_created of this HistoricalImportStatus.
        Date in which the historical import is initiated. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :param date_created: The date_created of this HistoricalImportStatus.
        :type: datetime
        """
        

        self._date_created = date_created

    @property
    def date_modified(self) -> datetime:
        """
        Gets the date_modified of this HistoricalImportStatus.
        Date in which the historical import is modified. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :return: The date_modified of this HistoricalImportStatus.
        :rtype: datetime
        """
        return self._date_modified

    @date_modified.setter
    def date_modified(self, date_modified: datetime) -> None:
        """
        Sets the date_modified of this HistoricalImportStatus.
        Date in which the historical import is modified. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :param date_modified: The date_modified of this HistoricalImportStatus.
        :type: datetime
        """
        

        self._date_modified = date_modified

    @property
    def active(self) -> bool:
        """
        Gets the active of this HistoricalImportStatus.
        Whether this historical import is active or not

        :return: The active of this HistoricalImportStatus.
        :rtype: bool
        """
        return self._active

    @active.setter
    def active(self, active: bool) -> None:
        """
        Sets the active of this HistoricalImportStatus.
        Whether this historical import is active or not

        :param active: The active of this HistoricalImportStatus.
        :type: bool
        """
        

        self._active = active

    @property
    def type(self) -> str:
        """
        Gets the type of this HistoricalImportStatus.
        Whether this historical import is of type csv or json

        :return: The type of this HistoricalImportStatus.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type: str) -> None:
        """
        Sets the type of this HistoricalImportStatus.
        Whether this historical import is of type csv or json

        :param type: The type of this HistoricalImportStatus.
        :type: str
        """
        if isinstance(type, int):
            type = str(type)
        allowed_values = ["Csv", "Json"]
        if type.lower() not in map(str.lower, allowed_values):
            # print("Invalid value for type -> " + type)
            self._type = "outdated_sdk_version"
        else:
            self._type = type

    def to_dict(self):
        """
        Returns the model properties as a dict
        """
        result = {}

        for attr, _ in iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value

        return result

    def to_json(self):
        """
        Returns the model as raw JSON
        """
        return json.dumps(sanitize_for_serialization(self.to_dict()))

    def to_str(self):
        """
        Returns the string representation of the model
        """
        return pformat(self.to_dict())

    def __repr__(self):
        """
        For `print` and `pprint`
        """
        return self.to_str()

    def __eq__(self, other):
        """
        Returns true if both objects are equal
        """
        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """
        Returns true if both objects are not equal
        """
        return not self == other

