# coding: utf-8

"""
Copyright 2016 SmartBear Software

    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at

        http://www.apache.org/licenses/LICENSE-2.0

    Unless required by applicable law or agreed to in writing, software
    distributed under the License is distributed on an "AS IS" BASIS,
    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
    See the License for the specific language governing permissions and
    limitations under the License.

    Ref: https://github.com/swagger-api/swagger-codegen
"""

from datetime import datetime
from datetime import date
from pprint import pformat
import re
import json

from ..utils import sanitize_for_serialization

# type hinting support
from typing import TYPE_CHECKING
from typing import List
from typing import Dict

if TYPE_CHECKING:
    from . import DecisionTableRowParameterValue
    from . import DecisionTableVersionEntity

class DecisionTableRow(object):
    """
    NOTE: This class is auto generated by the swagger code generator program.
    Do not edit the class manually.
    """
    def __init__(self) -> None:
        """
        DecisionTableRow - a model defined in Swagger

        :param dict swaggerTypes: The key is attribute name
                                  and the value is attribute type.
        :param dict attributeMap: The key is attribute name
                                  and the value is json key in definition.
        """
        self.swagger_types = {
            'id': 'str',
            'table': 'DecisionTableVersionEntity',
            'row_index': 'int',
            'date_created': 'datetime',
            'date_modified': 'datetime',
            'inputs': 'dict(str, DecisionTableRowParameterValue)',
            'outputs': 'dict(str, DecisionTableRowParameterValue)',
            'self_uri': 'str'
        }

        self.attribute_map = {
            'id': 'id',
            'table': 'table',
            'row_index': 'rowIndex',
            'date_created': 'dateCreated',
            'date_modified': 'dateModified',
            'inputs': 'inputs',
            'outputs': 'outputs',
            'self_uri': 'selfUri'
        }

        self._id = None
        self._table = None
        self._row_index = None
        self._date_created = None
        self._date_modified = None
        self._inputs = None
        self._outputs = None
        self._self_uri = None

    @property
    def id(self) -> str:
        """
        Gets the id of this DecisionTableRow.
        The globally unique identifier for the object.

        :return: The id of this DecisionTableRow.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id: str) -> None:
        """
        Sets the id of this DecisionTableRow.
        The globally unique identifier for the object.

        :param id: The id of this DecisionTableRow.
        :type: str
        """
        

        self._id = id

    @property
    def table(self) -> 'DecisionTableVersionEntity':
        """
        Gets the table of this DecisionTableRow.
        The decision table to which this row belongs

        :return: The table of this DecisionTableRow.
        :rtype: DecisionTableVersionEntity
        """
        return self._table

    @table.setter
    def table(self, table: 'DecisionTableVersionEntity') -> None:
        """
        Sets the table of this DecisionTableRow.
        The decision table to which this row belongs

        :param table: The table of this DecisionTableRow.
        :type: DecisionTableVersionEntity
        """
        

        self._table = table

    @property
    def row_index(self) -> int:
        """
        Gets the row_index of this DecisionTableRow.
        The absolute index of this row in the decision table, starting at 0

        :return: The row_index of this DecisionTableRow.
        :rtype: int
        """
        return self._row_index

    @row_index.setter
    def row_index(self, row_index: int) -> None:
        """
        Sets the row_index of this DecisionTableRow.
        The absolute index of this row in the decision table, starting at 0

        :param row_index: The row_index of this DecisionTableRow.
        :type: int
        """
        

        self._row_index = row_index

    @property
    def date_created(self) -> datetime:
        """
        Gets the date_created of this DecisionTableRow.
        The date when this row was created. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :return: The date_created of this DecisionTableRow.
        :rtype: datetime
        """
        return self._date_created

    @date_created.setter
    def date_created(self, date_created: datetime) -> None:
        """
        Sets the date_created of this DecisionTableRow.
        The date when this row was created. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :param date_created: The date_created of this DecisionTableRow.
        :type: datetime
        """
        

        self._date_created = date_created

    @property
    def date_modified(self) -> datetime:
        """
        Gets the date_modified of this DecisionTableRow.
        The date when this row was last modified. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :return: The date_modified of this DecisionTableRow.
        :rtype: datetime
        """
        return self._date_modified

    @date_modified.setter
    def date_modified(self, date_modified: datetime) -> None:
        """
        Sets the date_modified of this DecisionTableRow.
        The date when this row was last modified. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z

        :param date_modified: The date_modified of this DecisionTableRow.
        :type: datetime
        """
        

        self._date_modified = date_modified

    @property
    def inputs(self) -> Dict[str, 'DecisionTableRowParameterValue']:
        """
        Gets the inputs of this DecisionTableRow.
        The map input values of the row being created. At least one value must be provided. The key for this map is the column ID the row value relates

        :return: The inputs of this DecisionTableRow.
        :rtype: dict(str, DecisionTableRowParameterValue)
        """
        return self._inputs

    @inputs.setter
    def inputs(self, inputs: Dict[str, 'DecisionTableRowParameterValue']) -> None:
        """
        Sets the inputs of this DecisionTableRow.
        The map input values of the row being created. At least one value must be provided. The key for this map is the column ID the row value relates

        :param inputs: The inputs of this DecisionTableRow.
        :type: dict(str, DecisionTableRowParameterValue)
        """
        

        self._inputs = inputs

    @property
    def outputs(self) -> Dict[str, 'DecisionTableRowParameterValue']:
        """
        Gets the outputs of this DecisionTableRow.
        The map output values of the row being created. At least one value must be provided. The key for this map is the column ID the row value relates

        :return: The outputs of this DecisionTableRow.
        :rtype: dict(str, DecisionTableRowParameterValue)
        """
        return self._outputs

    @outputs.setter
    def outputs(self, outputs: Dict[str, 'DecisionTableRowParameterValue']) -> None:
        """
        Sets the outputs of this DecisionTableRow.
        The map output values of the row being created. At least one value must be provided. The key for this map is the column ID the row value relates

        :param outputs: The outputs of this DecisionTableRow.
        :type: dict(str, DecisionTableRowParameterValue)
        """
        

        self._outputs = outputs

    @property
    def self_uri(self) -> str:
        """
        Gets the self_uri of this DecisionTableRow.
        The URI for this object

        :return: The self_uri of this DecisionTableRow.
        :rtype: str
        """
        return self._self_uri

    @self_uri.setter
    def self_uri(self, self_uri: str) -> None:
        """
        Sets the self_uri of this DecisionTableRow.
        The URI for this object

        :param self_uri: The self_uri of this DecisionTableRow.
        :type: str
        """
        

        self._self_uri = self_uri

    def to_dict(self):
        """
        Returns the model properties as a dict
        """
        result = {}

        for attr, _ in self.swagger_types.items():
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value

        return result

    def to_json(self):
        """
        Returns the model as raw JSON
        """
        return json.dumps(sanitize_for_serialization(self.to_dict()))

    def to_str(self):
        """
        Returns the string representation of the model
        """
        return pformat(self.to_dict())

    def __repr__(self):
        """
        For `print` and `pprint`
        """
        return self.to_str()

    def __eq__(self, other):
        """
        Returns true if both objects are equal
        """
        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """
        Returns true if both objects are not equal
        """
        return not self == other

