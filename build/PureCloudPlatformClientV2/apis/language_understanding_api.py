...
class LanguageUnderstandingApi(object):
    ...
    def get_languageunderstanding_settings(self, **kwargs) -> 'NluOrganization':
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_languageunderstanding_settings(callback=callback_function)
        ...
        for key, val in params['kwargs'].items():
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_languageunderstanding_settings" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        resource_path = '/api/v2/languageunderstanding/settings'.replace('{format}', 'json')
        path_params = {}
