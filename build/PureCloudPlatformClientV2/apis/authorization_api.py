...
from ..models import DomainOrgRoleDifference
...
class AuthorizationApi(object):
    ...
    def put_authorization_roles_settings(self, body: 'RoleSettings', **kwargs) -> 'RoleSettings':
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.put_authorization_roles_settings(body, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param RoleSettings body: Authorization Role Settings (required)
        :return: RoleSettings
                 If the method is called asynchronously,
                 returns the request thread.
        """
        ...
        for key, val in params['kwargs'].items():
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method put_authorization_roles_settings" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        # verify the required parameter 'body' is set
        if ('body' not in params) or (params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `put_authorization_roles_settings`")
