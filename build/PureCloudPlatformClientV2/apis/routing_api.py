...
from ..models import UpdateSkillDivisionRequest
...
class RoutingApi(object):
    ...
    def patch_routing_skill(self, skill_id: str, body: 'UpdateSkillDivisionRequest', **kwargs) -> 'RoutingSkill':
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.patch_routing_skill(skill_id, body, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param str skill_id: Skill ID (required)
        :param UpdateSkillDivisionRequest body: updateSkillDivisionRequest (required)
        :return: RoutingSkill
                 If the method is called asynchronously,
                 returns the request thread.
        """
        ...
        for key, val in params['kwargs'].items():
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method patch_routing_skill" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        # verify the required parameter 'skill_id' is set
        if ('skill_id' not in params) or (params['skill_id'] is None):
            raise ValueError("Missing the required parameter `skill_id` when calling `patch_routing_skill`")
        # verify the required parameter 'body' is set
        if ('body' not in params) or (params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `patch_routing_skill`")
