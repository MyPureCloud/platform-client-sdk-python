...
class PresenceApi(object):
    ...
    def get_user_presences_zoomphone(self, user_id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_user_presences_zoomphone(user_id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_user_presences_zoomphone" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        # verify the required parameter 'user_id' is set
        if ('user_id' not in params) or (params['user_id'] is None):
            raise ValueError("Missing the required parameter `user_id` when calling `get_user_presences_zoomphone`")


        resource_path = '/api/v2/users/{userId}/presences/zoomphone'.replace('{format}', 'json')
        path_params = {}
        if 'user_id' in params:
            path_params['userId'] = params['user_id']
