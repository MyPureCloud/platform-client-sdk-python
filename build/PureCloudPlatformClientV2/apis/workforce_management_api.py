...
class WorkforceManagementApi(object):
    ...
    def get_workforcemanagement_team_adherence(self, team_id: str, **kwargs) -> 'UserScheduleAdherenceListing':
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_workforcemanagement_team_adherence(team_id, callback=callback_function)
        ...
        for key, val in params['kwargs'].items():
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_workforcemanagement_team_adherence" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        if ('team_id' not in params) or (params['team_id'] is None):
            raise ValueError("Missing the required parameter `team_id` when calling `get_workforcemanagement_team_adherence`")


        resource_path = '/api/v2/workforcemanagement/teams/{teamId}/adherence'.replace('{format}', 'json')
        path_params = {}
